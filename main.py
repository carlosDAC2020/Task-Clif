...
print(f"PubMed API Key: {'*' * (len(pubmed_api_key)-4) + pubmed_api_key[-4:] if pubmed_api_key else 'No encontrada'}")

gemini_api_key = os.getenv("GEMINI_API_KEY")
print(f"Gemini API Key: {'*' * (len(gemini_api_key)-4) + gemini_api_key[-4:] if gemini_api_key else 'No encontrada'}")
...
try:
    ...
    gemini_llm_instance = genai.GenerativeModel(gemini_model_name, safety_settings=safety_settings_config)
...
class LTV_Entitye_Clasifier_Local: # Placeholder
    def __init__(self, ner, pos): print("Placeholder LTV_Entitye_Clasifier_Local inicializado.")
    def get(self, text): print(f"Placeholder LTV.get() llamado para: '{text[:50]}...'"); return {"keywords": ["placeholder_kw1", "local_term"]}
classifier = LTV_Entitye_Clasifier_Local(ner_pipeline, pos_pipeline)
#---- FIN SIMULACIÓN ----

#--- Listas de Stop Words ---
stop_words_generales = set(['is','a','the','and','or','of','to','in','it','that','this','for','what','can','be','an','are','do','does','did','have','has','had','was','were','will','how','why','when','where','which','who','with','as','at','by','from','if','into','like','near','on','onto','out','over','past','than','then','through','under','until','up','upon','without','you','your','i','me','my','we','our','us','he','him','his','she','her','they','them','their', 'list', 'describe', 'main', 'give'])
stop_words_medicas_genericas = set(['disease','disorder','syndrome','patient','study','analysis','results','review','case','report','effect','treatment','therapy','clinical','trial','evidence','role','mechanism','approach','management','association','factor','risk','level','group','use','related','potential','impact','efficacy','safety','comparison','development','evaluation','assessment','status','marker','expression','pathway','interaction','cell','gene','protein','receptor','inhibitor','agonist','antagonist'])
...
INPUT_DATA_FOLDER = TRAIN_DATA_FOLDER if TYPE_EVALUATION == "TRAIN" else TEST_DATA_FOLDER
print(f"Carpeta de entrada: {INPUT_DATA_FOLDER}")
OUTPUT_DATA_FOLDER = RESULT_FOLDER_TRAIN if TYPE_EVALUATION == "TRAIN" else RESULT_FOLDER_TEST
...
METODO_EXTRACCION = 'LLM' if gemini_service.llm_model else 'LOCAL' # Decide basado en si el modelo se cargó
USAR_FALLBACK_LOCAL = True # ¿Intentar clasificador local si LLM falla?
USAR_REINTENTO_LLM = True if gemini_service.llm_model else False # ¿Reintentar búsqueda con LLM si la inicial falla?
CORE_TERMS_INICIAL = 1 # Core terms para la búsqueda híbrida inicial (cuando LLM o Local funcionó)
CORE_TERMS_LOCAL = 2   # Core terms si se usó solo el clasificador local (quizás menos preciso)
...
RERANKER_TYPE = os.getenv("RERANKER_TYPE") # Tipo de re-ranking a usar (ej: TF-IDF,  BM25  o PubMedBERT)
RERANKER_QUERY = os.getenv("RERANKER_QUERY") # Campo deL tipo de query a usar para el re-ranking el cual puede ser BODY, BODY + KEYWORDS o KEYWORDS
# ====================
...
print(f"Config: LLM Activo={bool(gemini_service.llm_model)}, Fallback Local={USAR_FALLBACK_LOCAL if METODO_EXTRACCION=='LLM' else 'N/A'}, Reintento Búsqueda LLM={USAR_REINTENTO_LLM}, Snippets Top={SNIPPETS_TOP_N}")
for i, item_original in enumerate(todas_las_preguntas):
    if i >= limite: break # Control de límite
    ...
    if not item_body:
        ...
        if PAUSA_ENTRE_ITEMS > 0: time.sleep(PAUSA_ENTRE_ITEMS)
    ...
    if METODO_EXTRACCION == 'LLM':
        ...
        keywords_llm = gemini_service.extract_keywords(item_body) # La pausa está dentro
        if keywords_llm:
            ...
            metodo_exitoso_extraccion = 'LLM_FALLIDO_A_LOCAL' # Marcar para intentar local
        else:
    ...
    if metodo_exitoso_extraccion == 'LOCAL' or metodo_exitoso_extraccion == 'LLM_FALLIDO_A_LOCAL':
        try:
            # Usar el clasificador local (Placeholder o real)
            result_body = classifier.get(item_body) # Asume que esto es rápido y no necesita pausa larga
            ...
            if keywords_local:
                ...
                metodo_exitoso_extraccion = 'LOCAL (Fallback)' if metodo_exitoso_extraccion == 'LLM_FALLIDO_A_LOCAL' else 'LOCAL'
            else:
                print("[!] Extracción local no devolvió keywords.")
                # Si era fallback y falló, contar como fallo total de extracción
                if metodo_exitoso_extraccion == 'LOCAL (Fallback)': num_fallos_extraccion_total += 1
    ...
    end_extr = time.time()
    print(f"[*] Keywords Extraídos ({metodo_exitoso_extraccion}): {keywords_para_filtrar} (Tiempo Extr: {end_extr - start_extr:.2f}s)")

    # Si no hay keywords después de intentar todo, crear item vacío y continuar
    if not keywords_para_filtrar:
        print(f"[!] No se pudieron extraer keywords para {item_id}. Creando entrada vacía.")
        output_item = item_original.copy(); output_item["documents"] = []; output_item["snippets"] = []
        resultados_finales_json["questions"].append(output_item)
        item_end_time = time.time(); tiempos_totales.append(item_end_time - item_start_time)
        print(f"[*] Item {item_id} procesado (vacío) en {item_end_time - item_start_time:.2f}s.")
        print("="*80)
        if PAUSA_ENTRE_ITEMS > 0: time.sleep(PAUSA_ENTRE_ITEMS)
    ...
    palabras_filtradas = [
        p.lower() for p in keywords_para_filtrar
        if isinstance(p, str) and # Asegurar que es string
        p.lower() not in stop_words_generales and
        p.lower() not in stop_words_medicas_genericas and
        len(p) > 1 and # Longitud mínima
        any(c.isalnum() for c in p) # Al menos un alfanumérico
    ]
    ...
    if not palabras_unicas_ordenadas:
        ...
        num_filtrado_cero += 1
        necesita_reintento_busqueda = True # Marcar para posible reintento LLM aunque no hubiera keywords
    else:
        # Decidir cuántos core terms usar basado en el éxito de la extracción
        num_core = CORE_TERMS_INICIAL if 'FALLIDO' not in metodo_exitoso_extraccion else CORE_TERMS_LOCAL
    ...
    pmids_para_procesar = pmids_encontrados_inicial # Empezar con los resultados iniciales
    ...
    if necesita_reintento_busqueda and USAR_REINTENTO_LLM:
        ...
        if nueva_consulta:
            ...
            pmids_para_procesar = resultados_pubmed_reintento.get('pmids', []) # Actualizar PMIDs a procesar
            if not pmids_para_procesar:
                print("[!] Reintento LLM con búsqueda directa tampoco encontró PMIDs.")
                num_reintentos_llm_fallidos_output += 1 # Contar fallo si no produjo resultados
            else:
                print(f"[*] Reintento LLM encontró {len(pmids_para_procesar)} PMIDs.")
        else:
            print("[!] LLM no pudo generar consulta reestructurada o la consulta era inválida.")
            num_reintentos_llm_fallidos_output += 1 # Contar fallo si no generó consulta
    ...
    if not pmids_para_procesar and not (necesita_reintento_busqueda and USAR_REINTENTO_LLM):
        # Esto cubre: inicial 0 sin reintento, o inicial 0 con reintento fallido
        if not necesita_reintento_busqueda: # Si la inicial ya dio 0 y no se marcó reintento
            num_busqueda_cero_pmids += 1
    ...
    if pmids_para_procesar:
        print(f"[*] Obteniendo detalles para {len(pmids_para_procesar)} PMIDs vía EFetch...")
        # Usar el servicio PubMed para fetch
        pmid_details = pubmed_service.fetch_details(pmids_para_procesar) # Pausa y reintentos dentro

        # Verificar si EFetch recuperó detalles para todos los PMIDs solicitados
        if len(pmid_details) < len(pmids_para_procesar):
            print(f"[!] Advertencia: EFetch recuperó detalles para {len(pmid_details)} de {len(pmids_para_procesar)} PMIDs.")
        ...
        if pmid_details: # Solo re-rankear si obtuvimos algún detalle
            ...
            if RERANKER_TYPE == "TF-IDF":
                pmids_finales_reranked, pmid_scores_map = reranker.rerank_TF_IDF(
                    original_question=query_rerank,
                    pmid_details_map=pmid_details
                )
            ...
            print("[!] No se obtuvieron detalles de PubMed (EFetch falló completamente?), no se puede re-rankear.")
            # Mantener los PMIDs originales si EFetch falló pero la búsqueda no
            pmids_finales_reranked = pmids_para_procesar
            pmid_scores_map = {pmid: 0.0 for pmid in pmids_finales_reranked} # Scores a cero

    else:
        print("[!] No hay PMIDs para obtener detalles o reordenar (Búsqueda inicial/reintento falló).")
    ...
    output_item["documents"] = [f"http://www.ncbi.nlm.nih.gov/pubmed/{pmid}" for pmid in final_documents_list] # Formato URL
    output_item["snippets"] = []

    print(f"[*] Generando snippets para los Top {len(final_documents_list)} documentos...")
    for rank, pmid in enumerate(final_documents_list, 1):
        details = pmid_details.get(str(pmid)) # Asegurar que pmid es string para buscar en dict
        ...
        if details:
            ...
            if title and title != "N/A" and abstract and abstract != "N/A":
                ...
                begin_sec, end_sec = "abstract", "abstract" # O considerar "title_abstract"
    ...
    print(f"[*] Item {item_id} procesado en {item_duration:.2f}s. Consulta: {consulta_usada_finalmente}. Documentos/Snippets en JSON: {len(output_item['documents'])}")
    print("\n" + "=" * 80)
...
print("-" * 20)
print(f"Fallos Totales en Extracción Inicial de Keywords: {num_fallos_extraccion_total} ({num_fallos_extraccion_total / num_items_procesados if num_items_procesados else 0:.1%})")
print(f"Items con Cero Keywords Válidos Post-Filtrado: {num_filtrado_cero} ({num_filtrado_cero / num_items_procesados if num_items_procesados else 0:.1%})")
...
print(f"Items Resultando en Cero PMIDs Tras Búsqueda(s): {total_busquedas_sin_pmids} ({total_busquedas_sin_pmids / num_items_procesados if num_items_procesados else 0:.1%})")
print("-" * 20)
print(f"Reintentos de Búsqueda con LLM Iniciados: {num_reintentos_llm_iniciados}")
print(f"Reintentos LLM Fallidos (No generó consulta o no encontró PMIDs): {num_reintentos_llm_fallidos_output}")
print(f"Items con Fallos Parciales en EFetch (No todos los detalles recuperados): {num_efetch_fallidos_parcial}")
print("-" * 20)
if tiempos_totales:
...
if resultados_finales_json["questions"]:
    ...
    print(json.dumps(resultados_finales_json["questions"][0], indent=2, ensure_ascii=False))
else:
...
#gyardar la grafica
plt.savefig(f"{FOLDER_METRICS}/{TYPE_EVALUATION}_metrics_{RERANKER_TYPE}_{fecha_hora_actual}.png")